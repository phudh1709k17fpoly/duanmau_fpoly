...
if ($_SERVER['REQUEST_METHOD'] == "POST"){
 require_once "../model/mahoa.php";
  ...
  if (empty($name)) {
    $err_ms['name'] = "bạn không được bỏ trống name";
  }
  if (empty($email)) {
    $err_ms['email'] = "bạn không được bỏ trống email";
  }
  if (empty($username)) {
  ...
  if(!empty($checkpass)){
    if($password == $checkpass){
      ...
      $sql = "INSERT INTO `manage_login`(`name`, `username`, `password`, `email`,`password_mahoa`)
       VALUES ('$name','$username','$password_ed','$email','$password');";
